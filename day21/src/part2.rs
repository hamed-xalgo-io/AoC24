...
fn dirpad_all_possible_moves() -> HashMap<(char, char), Vec<String>> {
    ...
    dirpad.chars().for_each(|s| {
        possible_moves.insert((s, s), vec!["".to_string()]);
    });
    possible_moves
}
...
fn main() {
    ...
    for lvl in 1..26 {
        for start in "A^<v>".chars() {
            for press in "A^<v>".chars() {
                let start_to_press = &dirpad_possible_moves[&(start, press)];
                if lvl == 1 {
                    cache.insert((start, press, lvl), start_to_press[0].len() + 1);
                } else {
                    let mut min = usize::MAX;
                    for path in start_to_press.iter() {
                        let path = format!("A{path}A");
                        let mut path_length = 0;
                        for (s, d) in path.chars().tuple_windows() {
                            path_length += cache[&(s, d, lvl - 1)];
                        }
                        min = min.min(path_length);
                    }
                    cache.insert((start, press, lvl), min);
                }
            }
        }
    }
    ...
    for passcode in passcodes {
        ...
        for path in paths.iter() {
            let mut path_length = 0;
            for (s, d) in path.chars().tuple_windows() {
                path_length += cache[&(s, d, 25)];
            }
            best = best.min(path_length);
        }
